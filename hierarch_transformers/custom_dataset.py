...
class Custom_Dataset_Class(Dataset):
    ...
    def __init__(self, tokenizer, max_len, chunk_len=200, overlap_len=50, approach="all", max_size_dataset=None, file_location="combined_data.csv", min_len=249):
    ...
    def process_data(self, file_location):
        ...
        train_raw = df.assign(
            len_txt=df.text.apply(lambda x: len(x.split())))
        train_raw.reset_index(inplace=True, drop=True)
        train_raw = train_raw.reindex(np.random.permutation(train_raw.index))
        return train_raw['text'].values, train_raw['label'].values
    ...
    def long_terms_tokenizer(self, data_tokenize, targets):
        ...
        if self.approach != 'head':
            remain = torch.tensor(remain, dtype=torch.long)[0]
